...
class MAPPO(Policy):
    def __init__(
        ...
        **kwargs,
        ...
        # LLM penalty is now injected by GRFLLMMaskWrapper at the env level and
        # passed through the episode buffer as EpisodeKey.LLM_PENALTY.  The policy
        # reads it from kwargs — no stateful entropy mask or 1-step lag here.
    ...
    def compute_action(self, **kwargs):
        ...
        with torch.set_grad_enabled(not inference):
            ...
            # LLM penalty: read directly from kwargs.
            # During rollout (inference=True): provided by GRFLLMMaskWrapper via the
            #   env's step/reset return dict, stored in step_data, and passed as
            #   **policy_inputs.  Warmup suppression (zeros before warmup_epochs) is
            #   handled in the wrapper — the policy is agnostic to epoch count.
            # During training (inference=False): the same penalty that was stored in
            #   the episode buffer is passed via compute_action_kwargs in loss.py,
            #   ensuring old_log_prob and new_log_prob use the same distribution.
            _penalty_to_use = kwargs.get(EpisodeKey.LLM_PENALTY, None)

            if _penalty_to_use is not None and hasattr(self.actor, 'logits'):
                ...
                raw_logits = raw_logits - 1e10 * (1 - action_masks) - _penalty_to_use
