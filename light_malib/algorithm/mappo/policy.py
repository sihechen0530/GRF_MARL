...
class MAPPO(Policy):
    def __init__(
        ...
        **kwargs,
        ...
        # Optional LLM-guided entropy masking (enable via custom_config["use_llm_mask"])
        if custom_config.get("use_llm_mask", False):
            self.entropy_mask = EntropyGuidedMask(
                ...
                penalty_scale=custom_config.get("llm_mask_penalty_scale", 3.0),
        ...
        # Carries the LLM penalty tensor from step t to step t+1 (1-step lag)
        self._llm_penalty_tensor = None
    ...
    def to_device(self, device):
        ...
        self_copy._llm_penalty_tensor = None  # reset; will be rebuilt on first inference step
        if self.share_backbone:
    ...
    def compute_action(self, **kwargs):
        ...
        with torch.set_grad_enabled(not inference):
            ...
            if self._llm_penalty_tensor is not None and hasattr(self.actor, 'logits'):
                # Apply soft LLM penalty directly on logits to avoid float32 precision
                # loss from encoding into action_masks (1e10 scale vs 1-3 penalty scale).
                # actor.logits() is defined on the common Actor base class.
                raw_logits, actor_rnn_states = self.actor.logits(
                    observations, actor_rnn_states, rnn_masks
                )
                raw_logits = raw_logits - 1e10 * (1 - action_masks) - self._llm_penalty_tensor
            ...
            # Update entropy mask with this step's entropy and regime (inference only).
            # The resulting penalty is stored and used on the NEXT step (1-step lag).
            if inference and self.entropy_mask is not None and dist_entropy is not None and getattr(self, "_entropy_mask_enabled", True):
                ...
                self.entropy_mask.step(entropy_val, regime)
                penalty_np = self.entropy_mask.get_penalty()
                if penalty_np is not None:
                    self._llm_penalty_tensor = torch.tensor(
                        penalty_np, dtype=torch.float32, device=self.device
                    )
                else:
                    self._llm_penalty_tensor = None
            ...
            if kwargs.get(EpisodeKey.ACTION,None) is None:
                ...
                ret[EpisodeKey.ACTION_ENTROPY]=dist_entropy  # always non-None now
    ...
    def train(self):
        ...
        if self.share_backbone:
            self.backbone.train()
        self._entropy_mask_enabled = True
    ...
    def eval(self):
        ...
        if self.share_backbone:
            self.backbone.eval()
        self._entropy_mask_enabled = False
